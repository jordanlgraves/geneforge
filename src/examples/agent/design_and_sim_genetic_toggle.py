...
class GeneticToggleSwitchExample(ExampleRunner):
...
def run_example():
    ...
    Returns the ExampleRunner instance so that callers can inspect its messages/state.
    """
    # Create and run the example using the reusable harness
    runner = GeneticToggleSwitchExample(
        example_name="Genetic Toggle Switch",
        prompt=PROMPT,
        max_rounds=15,
        max_attempts=2,
    )
    ...
    runner.log_results(final_response)

    # Return the runner for external inspection
    return runner
